...
import { CancelRentalContractUseCase } from './cancel-rental-contract.use-case'
...
describe('CancelRentalContractUseCase', () => {
  ...
  let useCase: CancelRentalContractUseCase
  ...
  beforeEach(async () => {
    ...
    useCase = new CancelRentalContractUseCase(
      contractsRepo,
      propertiesRepo,
      usersRepo,
    )
  ...
  it('should cancel a PENDING contract', async () => {
    ...
    expect(result?.cancelReason).toBe('Client backed out')
  ...
  it('should throw if user is not admin', async () => {
    ...
    ).rejects.toBeInstanceOf(OnlyAdminCanCancelContractError)
  })
  ...
  it('should throw if contract status cannot be cancelled', async () => {
